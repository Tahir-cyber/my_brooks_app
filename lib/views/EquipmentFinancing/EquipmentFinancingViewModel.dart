...
class EquipmentFinancingViewModel extends ChangeNotifier {
  bool loading = false;
  PlatformFile? file;
  PlatformFile? file2;
  PlatformFile? file3;
  ...
  uploadFileToStorageMethod(
    ...
    try {
      final response = await FirebaseStorage.instance
          .ref()
          .child(
              "EquipFinanDoc/${DateTime.now().millisecondsSinceEpoch.toString()}")
          .putFile(File(file));
  ...
  uploadFileToFirestore() async {
    ...
    await FirebaseFirestore.instance
        ...
        .collection("equipmentFinancingDoc")
        .add({
      "pdfurl": pdfUrlList,
      "pdfurl2": pdfUrlList2,
      "pdfurl3": pdfUrlList3,
      ...
      result8 != null ? result8!.files.length = 0 : null;
      result9 != null ? result9!.files.length = 0 : null;
      result10 != null ? result10!.files.length = 0 : null;
      result11 != null ? result11!.files.length = 0 : null;

      Get.to(HardMoneyFinancingScreen());
    });
  }
}
